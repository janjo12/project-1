...
import type { RoomSceneActor, ScenePosition } from "@/components/game-view-panel";
...
function Game({ settings, onGameOver, onExit }: { settings: GameSettings; onGameOver: (score: number) => void; onExit: () => void }) {
  ...
  function actorPress(actor: RoomSceneActor) {
  ...
  function doorwayPress(position: Exclude<ScenePosition, "center">) {
  ...
  return <main className={`shell game-shell ${dark ? "dark" : "light"}`}>
    <section className="game-card"><header className="game-header"><strong>Dungeon Run · Level {game.level}</strong><button className="quiet" onClick={() => setPaused(true)}>Menu</button></header>
      ...
      <GameViewPanel sceneFrameStore={game.sceneFrameStore} canUnlockDoors={game.inventoryItem === "key"} disabled={game.isResolving || game.hasLost} enemyHealthLossAmount={game.enemyHealthLossAmount} hardTurnCounter={game.hardTurnCounter} onActorPress={actorPress} onDoorwayPress={doorwayPress} onPlayerPress={game.defend} playerPosition={game.playerScenePosition} roomId={game.currentRoomId} reducedMotion={settings.reducedMotion} roomDoorways={game.roomDoorways} roomSceneActors={game.roomSceneActors} playerEnergyLossAmount={game.playerEnergyLossAmount} playerHealthLossAmount={game.playerHealthLossAmount}/>
    </section>
